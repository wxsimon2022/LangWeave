"""Rate-limit middleware (moved from ``langweave.web.middleware`` to break framework dependency).

Owns the ``RateLimitMiddleware`` class that depends on ``app.infrastructure.cache``.
``SecurityHeadersMiddleware`` remains in ``langweave.web.middleware`` as framework-neutral code.
"""
