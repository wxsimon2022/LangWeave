...
```bash
curl -X POST http://127.0.0.1:30002/api/v1/unified/stream \
...
```bash
curl -X POST http://127.0.0.1:30002/api/v1/agents/assistant/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "12 * 8 = ?"}'
```
...
- 本地 dev 将 `/api` 代理到 `http://localhost:30002`；`VITE_API_BASE_URL` 留空即可
